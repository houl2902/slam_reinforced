...
//     MatrixOperations::matrixTranspose(F, FT);
//     double temp[STATE_SIZE][STATE_SIZE];
//     MatrixOperations::matrixMultiply(F, covariance_matrix, temp);
//     MatrixOperations::matrixMultiply(temp, FT, covariance_matrix);
//     MatrixOperations::matrixAdd(covariance_matrix, motion_noise, covariance_matrix);
// }

// void EKFslam::update(double measurement[2]) {
//     // Матрица Якоби для модели наблюдения (H)
//     double H[2][STATE_SIZE] = {0};
//     H[0][0] = -std::cos(state[2]);
//     H[0][1] = -std::sin(state[2]);
//     H[1][0] = std::sin(state[2]) / (state[0] * state[0] + state[1] * state[1]);
//     H[1][1] = -std::cos(state[2]) / (state[0] * state[0] + state[1] * state[1]);
...
//     MatrixOperations::matrixTranspose(H, HT);

//     double S[2][2];
//     double temp[STATE_SIZE][STATE_SIZE];
//     MatrixOperations::matrixMultiply(H, covariance_matrix, temp);
//     MatrixOperations::matrixMultiply(temp, HT, S);
//     MatrixOperations::matrixAdd(S, measurement_noise, S);
...
//     MatrixOperations::matrixMultiply(covariance_matrix, HT, temp);
//     MatrixOperations::matrixMultiply(temp, S_inv, K);
...
//     MatrixOperations::matrixMultiply(K, H, KH);
//     MatrixOperations::matrixSubtract(I, KH, temp);
//     MatrixOperations::matrixMultiply(temp, covariance_matrix, covariance_matrix);
// }